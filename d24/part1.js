...
async function processLineByLine() {
  ...
  while (stack.length > 0) {
    ...
    if (posI + 1 < inputLines.length && !boards[boardId][posI + 1][posJ] && minutes + 1 < bestTimes[boardId][posI + 1][posJ]) {
      if (posI + 1 !== inputLines.length - 1) {
        stack.push([posI + 1, posJ, minutes + 1]);
      }
      bestTimes[boardId][posI + 1][posJ] = minutes + 1;
    }
  ...
  for (const bestTimesForBoard of bestTimes) {
    if (bestTimesForBoard[inputLines.length - 1][inputLines[0].length - 2] < bestTime) {
      bestTime = bestTimesForBoard[inputLines.length - 1][inputLines[0].length - 2];
    }
  }
  // for (const bestTimesForBoard of bestTimes) {
